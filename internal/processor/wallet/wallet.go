...
func (a *Adapter) ProcessWallet(ctx context.Context, req processor.WalletRequest) (*processor.ProcessorResponse, error) {
	body, err := json.Marshal(req)
	if err != nil {
		return nil, fmt.Errorf("marshal wallet request: %w", err)
	}

	httpReq, err := http.NewRequestWithContext(ctx, http.MethodPost, a.baseURL+"/process/wallet", bytes.NewReader(body))
	if err != nil {
